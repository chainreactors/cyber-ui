...
export function MarkdownEditor({
  ...
  return (
    <div className={cn('relative overflow-hidden rounded-lg border border-border bg-card focus-within:ring-2 focus-within:ring-ring/30', className)}>
      <div className="flex min-h-8 flex-wrap items-center justify-between gap-2 border-b border-border bg-muted/25 px-2 py-1">
        <div className="flex flex-wrap items-center gap-0.5">
          {([
            ...
            <ToolButton key={action} title={labels[action]} disabled={disabled || textareaProps.readOnly || mode !== 'edit'} onClick={() => format(action)}><Icon className="h-3.5 w-3.5" aria-hidden="true" /></ToolButton>
          ))}
        </div>
        <div className="ml-auto flex shrink-0 items-center gap-1 rounded-md border border-border bg-background p-0.5">
          <ModeButton active={mode === 'edit'} onClick={() => setMode('edit')} title={labels.edit}><Pencil className="h-3.5 w-3.5" aria-hidden="true" /></ModeButton>
      ...
      />
      {mode === 'preview' && (
        <div role="region" tabIndex={0} aria-label={labels.preview} className="max-h-96 overflow-auto bg-background px-4 py-3 text-sm [overflow-wrap:anywhere]" style={{ minHeight }}>
          {value.trim() ? <MarkdownContent content={value} compact={compact} /> : <p className="text-muted-foreground">{labels.empty}</p>}
        </div>
      )}
